...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
    ...
    Class.prototype.getStrWidth = function (str) {
        let el = document.getElementById('laytablecontentlength');
        if (!el) {
            el = document.createElement('span');
            el.style = 'visibility: hidden;';
            el.id = 'laytablecontentlength'
            ...
            $.ajax({
                ...
                , success: function (res) {
                    ...
                    if (res[response.statusName] != response.statusCode) {
                        ...
                        //科学计算自适应功能
                        if (that.config.cols.length == 1 && that.config.autoWidth == true) {
                            //获取每一列最大内容的宽度
                            let colsWidth = [];
                            let cols = that.config.cols[0];
                            for (var t = 0; t < res.data.length; t++) {
                                const temp = res.data[t];
                                for (var i = 0; i < cols.length; i++) {
                                    const col = cols[i];
                                    if (temp[col['field']] != null
                                        && col['minWidth'] == null
                                        && (!col['width'] || col['width'] < temp[col['field']].toString().length)) {
                                        colsWidth.push(col['field']);
                                        col['width'] = that.getStrWidth(temp[col['field']].toString());
                                    }
                                }
                            }
                            //获取table的总宽度
                            let tablewidth = $('.layui-table').width() - 10;
                            let temptablewidth = tablewidth;
                            //当前以用的宽度
                            let currwidth = 0;
                            //去除fixed的宽度
                            for (var i = 0; i < cols.length; i++) {
                                const temp = cols[i];
                                if (temp['fixed'] != null) {
                                    temptablewidth -= temp['width'];
                                }
                                else {
                                    currwidth += temp['width'];
                                }
                            }
                            //如果超出则不需要计算
                            if (currwidth < tablewidth) {
                                //把剩下的宽度平均分割到剩下的列
                                let exclude = cols.filter(x => x['fixed'] == null).length;
                                //计算非fixed列的遗产
                                let heritage = Math.round((temptablewidth - currwidth) / exclude);
                                //剩下的列继承一下遗产
                                currwidth = 0;
                                for (var i = 0; i < cols.length; i++) {
                                    const temp = cols[i];
                                ...
                                //继续计算遗产
                                heritage = tablewidth - currwidth;
                                let disabled = 0;
                                if (heritage > 0) { //遗产还有结余
                                    let money = null;
                                    for (var i = 0; i < cols.length; i++) {
                                        const temp = cols[i];
                                        if (temp['fixed'] == null && (money == null || money > temp['width'])) {
                                            money = temp['width'];
                                            disabled = i;
                                        }
                                    }
                            ...
                            for (var i = 0; i < cols.length; i++) {
                                const col = cols[i];
                                parent += '.layui-table-view .layui-table td[data-key="1-' + col['key'] + '"],.layui-table-view .layui-table th[data-key="1-' + col['key'] + '"]{';
                                style += '.laytable-cell-1-' + col['key'] + '{';
                                if (col['width'] && col['minWidth'] == null && colsWidth.indexOf(col['field']) > -1) {
                                    style += 'width:' + col['width'] + 'px;padding-right:5px;';
                                    parent += 'width:' + col['width'] + 'px;';
                                }
                                else if (col['width']) {
                                    parent += 'width:' + col['width'] + 'px;';
                            ...
                            for (var i = 0; i < cols.length; i++) {
                                const col = cols[i];
                                if (col['fixed']) {
                                    style += '.layui-table-view .layui-table td, .layui-table-view .layui-table th{ width:' + col['width'] + 'px; }';
                                }
                            }

                            Array.from(document.getElementsByTagName('style'))
                                .filter(x => x.innerHTML.indexOf('laytable-cell-') > -1)[0].innerHTML = style + parent;
                        }
            ...
            , render = function () { //后续性能提升的重点
                ...
                layui.each(data, function (i1, item1) {
                    ...
                    that.eachCols(function (i3, item3) {
                        ...
                        // Dengchao 2020-06-24 [ 设置空field的情况下只显示head ]
                        if (!item3.justhead || item3.justhead == false)
                            tds.push(td);
